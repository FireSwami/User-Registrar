...
class HelloView(FormView):
    ...
    def form_invalid(self, form):
        ...
        return redirect(reverse('already') + "?" + urllib.parse.urlencode(form.cleaned_data))
...
def hello_page(request):
    return render(request, 'hi/hello.html', {'user_data': request.session["user_data"], 'title': 'Страница приветствия'})


def already_page(request):
    return render(request, 'hi/already.html', {'user_data': request.session["user_data"], 'title': 'Страница уведомления'})
